...
(function ($) {
  ...
  window.ehfb = {
    ...
    addBodyClass: function addBodyClass() {
      $('body').addClass('has-ehfb-builder');
    },
    ...
    storeGlobals: function storeGlobals() {
      ...
      // Column Area.
      $(document).on('click mouseover', '.emoza-ehfb-area:not(.ehfb-available-components)', function (e) {
        if ($('#emoza-ehfb-elements').hasClass('show')) {
          return false;
        }
        _this.currentRowInput = $('#_customize-input-emoza_' + _this.currentBuilderType + '_row__' + $(this).data('ehfb-row'));
        _this.currentRow = $(this).closest('.emoza-ehfb-row');
        _this.currentColumnPos = $(this).index() - 1;
        _this.currentColumn = $(this);
        if (!_this.currentRow.length && $(this).hasClass('emoza-ehfb-area-offcanvas')) {
          _this.currentRowInput = $('#_customize-input-emoza_header_row__mobile_offcanvas');
          _this.currentRow = $('.emoza-ehfb-area-offcanvas');
          _this.currentColumnPos = $(this).index();
        }
      });
      $(document).on('click mouseover', '.ehfb-button', function (e) {
        _this.currentComponent = $(this).data('ehfb-id');
      });
    },
    devicesSwitcher: function devicesSwitcher() {
      ...
      $(' .wp-full-overlay-footer .devices button, .emoza-devices-preview button').on('click', function () {
        ...
        $('.emoza-ehfb-devices .emoza-ehfb-device-link').removeClass('active');
        $('.emoza-ehfb-devices .emoza-ehfb-device-link[data-device="' + device + '"]').addClass('active');
      });
      $('.emoza-ehfb-devices .emoza-ehfb-device-link').on('click', function (e) {
        e.preventDefault();
        var device = $(this).attr('data-device');
    ...
    elementsPopup: function elementsPopup() {
      var _this = this;
      $(document).on('click', '.emoza-ehfb-area:not(.ehfb-available-components)', function (e) {
        var popup = _this.currentBuilder.find('#emoza-ehfb-elements'),
          rect = $(this)[0].getBoundingClientRect(),
          row = $(this).data('ehfb-row');
        setTimeout(function () {
          popup.css('top', 0);
          ...
          if (e.target.classList.contains('ehfb-remove-element') || e.target.classList.contains('ehfb-button')) {
            return false;
          }
          popup.addClass('show');
        }, 200);
    ...
    closeElementsPopup: function closeElementsPopup(e) {
      var _this = this,
        popup = _this.currentBuilder.find('#emoza-ehfb-elements');
      if (e.target.closest('#emoza-ehfb-elements') === null) {
    ...
    elementsPopupContent: function elementsPopupContent() {
      ...
      var _this = this,
        ...
        elementsWrapper = $('.emoza-ehfb-elements-desktop'),
        mobileElementsWrapper = $('.emoza-ehfb-elements-mobile');
      ...
      if (elements.desktop.length) {
        ...
        try {
          for (_iterator7.s(); !(_step7 = _iterator7.n()).done;) {
            ...
            elementsWrapper.append('<div class="emoza-ehfb-element emoza-ehfb-element-desktop">' + '<a href="#" class="ehfb-button" data-ehfb-id="' + element.id + '" data-ehfb-focus-section="emoza_section_' + cprefix + '_component__' + element.id + '">' + element.label + '</a>' + '</div>');
          }
        } catch (err) {
          _iterator7.e(err);
        } finally {
        ...
        elementsWrapper.append('<p class="ehfb-elements-message">' + emoza_hfb.i18n.elementsMessage + '</p>');
      }
      ...
      if (elements.mobile.length) {
        ...
        try {
          for (_iterator8.s(); !(_step8 = _iterator8.n()).done;) {
            ...
            mobileElementsWrapper.append('<div class="emoza-ehfb-element emoza-ehfb-element-mobile">' + '<a href="#" class="ehfb-button" data-ehfb-id="' + _element.id + '" data-ehfb-focus-section="emoza_section_' + cprefix + '_component__' + _element.id + '">' + _element.label + '</a>' + '</div>');
          }
        } catch (err) {
          _iterator8.e(err);
        } finally {
        ...
        mobileElementsWrapper.append('<p class="ehfb-elements-message">' + emoza_hfb.i18n.elementsMessage + '</p>');
      }
    ...
    updateAvailableComponents: function updateAvailableComponents() {
      ...
      if (_this.currentBuilderType === 'header') {
        ...
        $('.emoza-header-builder-available-components').html($('.emoza-ehfb-header .emoza-ehfb-elements-desktop').html());

        // Header Mobile Components.
        $('.emoza-header-builder-available-mobile-components').html('');
        $('.emoza-header-builder-available-mobile-components').html($('.emoza-ehfb-header .emoza-ehfb-elements-mobile').html());
      }
      if (_this.currentBuilderType === 'footer') {
        // Footer Components.
        $('.emoza-footer-builder-available-footer-components').html('');
        $('.emoza-footer-builder-available-footer-components').html($('.emoza-ehfb-footer .emoza-ehfb-elements-desktop').html());
      }
    },
    addUpsellComponents: function addUpsellComponents() {
      ...
      var upsellHTML = "\n                <div class=\"emoza-ehfb-upsell-components-wrapper\">\n                    <h4>".concat(emoza_hfb.upsell_components.title, "</h4>\n                    <div class=\"emoza-ehfb-upsell-components\">").concat(upsellComponentsHTML, "</div>\n                    <p>").concat(emoza_hfb.upsell_components.total, "</p>\n                    <a href=\"").concat(emoza_hfb.upsell_components.link, "\" target=\"_blank\" class=\"ehfb-upsell-button\">").concat(emoza_hfb.upsell_components.button, "</a>\n                </div>\n            ");
      $('#emoza-ehfb-elements .emoza-ehfb-elements-wrapper .emoza-ehfb-upsell-components-wrapper').remove();
      $('#emoza-ehfb-elements .emoza-ehfb-elements-wrapper').append(upsellHTML);
    },
    elementsButton: function elementsButton() {
      var _this = this;
      $(document).on('click', '.emoza-ehfb-element > a', function (e) {
        e.preventDefault();
        e.stopPropagation();
        var id = $(this).data('ehfb-id'),
          focusSection = $(this).data('ehfb-focus-section');
        if ($(this).closest('#emoza-ehfb-elements').length) {
          _this.elementsButtonAdd(id);

          // close elements popup.
          _this.currentBuilder.find('#emoza-ehfb-elements').removeClass('show');
        } else {
          if (e.target.classList.contains('ehfb-remove-element')) {
    ...
    elementsButtonAdd: function elementsButtonAdd(id) {
      ...
      if (_this.currentDevice === 'mobile' && _this.currentRow.hasClass('emoza-ehfb-area-offcanvas')) {
      ...
      if (_this.currentComponent === 'mobile_offcanvas_menu' && !_this.currentRow.hasClass('emoza-ehfb-area-offcanvas')) {
        return false;
      }
      if (_this.currentComponent === 'mobile_hamburger' && _this.currentRow.hasClass('emoza-ehfb-area-offcanvas')) {
        return false;
      }
      ...
      _this.builderGridContent();
      $('#emoza-ehfb-elements').removeClass('show');
    },
    ...
    elementsSortable: function elementsSortable() {
      var _this = this;
      $('.emoza-ehfb-area').each(function () {
        $(this).sortable({
          placeholder: "emoza-ehfb-element ehfb-ui-state-highlight",
          connectWith: '.emoza-ehfb-area',
          scroll: false,
          cancel: '.ehfb-edit-column',
          change: function change(e, ui) {
            _this.currentComponent = $(ui.item[0]).find('.ehfb-button').data('ehfb-id');
            _this.currentRow = !$(ui.placeholder[0]).closest('.emoza-ehfb-row').length ? $(ui.placeholder[0]).closest('.emoza-ehfb-area-offcanvas') : $(ui.placeholder[0]).closest('.emoza-ehfb-row');
            _this.currentRowInput = $('#_customize-input-emoza_' + _this.currentBuilderType + '_row__' + ui.placeholder.closest('.emoza-ehfb-area').data('ehfb-row'));
            var order = [];
            ui.placeholder.closest('.emoza-ehfb-area').find('.ui-sortable-placeholder').attr('data-ehfb-id', _this.currentComponent);
            ui.placeholder.closest('.emoza-ehfb-area').find('.emoza-ehfb-element').each(function () {
              var cid = typeof $(this).find('.ehfb-button').data('ehfb-id') !== 'undefined' ? $(this).find('.ehfb-button').data('ehfb-id') : $(this).data('ehfb-id');
          ...
          update: function update(e, ui) {
            ...
            if (this === ui.item.parent()[0]) {
              var component_id = ui.item.find('> .ehfb-button').data('ehfb-id'),
                row = ui.item.closest('.emoza-ehfb-area').data('ehfb-row'),
                column = ui.item.closest('.emoza-ehfb-area').index() - 1,
                prevRow = ui.sender !== null ? ui.sender.data('ehfb-row') : null,
              ...
              if (!ui.sender.hasClass('ehfb-available-components')) {
    ...
    builderGridContent: function builderGridContent() {
      ...
      setTimeout(function () {
        for (var _i4 = 0, _fields2 = fields; _i4 < _fields2.length; _i4++) {
          ...
          // Empty columns.
          $('.emoza-ehfb-area[data-ehfb-row="' + current_row + '_' + _this.currentBuilderType + '_row"]').each(function () {
            $(this).remove();
          });
                ...
                if (columns.length) {
                  ...
                  try {
                    for (_iterator11.s(); !(_step11 = _iterator11.n()).done;) {
                      ...
                      column.append('<div class="emoza-ehfb-element">' + '<a href="#" class="ehfb-button" data-ehfb-id="' + _element2.id + '" data-ehfb-focus-section="emoza_section_' + cprefix + '_component__' + _element2.id + '">' + '<span class="ehfb-title-element">' + _element2.label + '</span>' + '<i class="ehfb-edit-element dashicons dashicons-admin-generic"></i>' + '<i class="ehfb-remove-element dashicons dashicons-no-alt"></i>' + '</a>' + '</div>');
                    }
                  } catch (err) {
                    _iterator11.e(err);
                  } finally {
          ...
          if (_this.currentDevice === 'mobile') {
            ...
            try {
              for (_iterator12.s(); !(_step12 = _iterator12.n()).done;) {
                ...
                var _column = $('.emoza-ehfb-' + current_row + '-row').find('.emoza-ehfb-area:last-child');
                if (_columns.length) {
                  ...
                  try {
                    for (_iterator14.s(); !(_step14 = _iterator14.n()).done;) {
                      ...
                      _column.append('<div class="emoza-ehfb-element">' + '<a href="#" class="ehfb-button" data-ehfb-id="' + _element3.id + '" data-ehfb-focus-section="emoza_section_' + cprefix + '_component__' + _element3.id + '">' + '<span class="ehfb-title-element">' + _element3.label + '</span>' + '<i class="ehfb-edit-element dashicons dashicons-admin-generic"></i>' + '<i class="ehfb-remove-element dashicons dashicons-no-alt"></i>' + '</a>' + '</div>');
                    }
                  } catch (err) {
                    _iterator14.e(err);
                  } finally {
            ...
            if (field.indexOf('mobile_offcanvas') !== -1) {
              $('.emoza-ehfb-area-offcanvas').html('');
              if (value.mobile_offcanvas.length) {
                ...
                try {
                  for (_iterator13.s(); !(_step13 = _iterator13.n()).done;) {
                    ...
                    $('.emoza-ehfb-area-offcanvas').append('<div class="emoza-ehfb-element">' + '<a href="#" class="ehfb-button" data-ehfb-id="' + element.id + '" data-ehfb-focus-section="emoza_section_hb_component__' + element.id + '">' + '<span class="ehfb-title-element">' + element.label + '</span>' + '<i class="ehfb-edit-element dashicons dashicons-admin-generic"></i>' + '<i class="ehfb-remove-element dashicons dashicons-no-alt"></i>' + '</a>' + '</div>');
                  }
                } catch (err) {
                  _iterator13.e(err);
                } finally {
        ...
        if (_this.currentBuilder.hasClass('show') && !_this.currentBuilder.hasClass('show-bottom')) {
          $('.emoza-ehfb').css('height', 0);
          _this.currentBuilder.css('height', _this.currentBuilder.find('.emoza-ehfb-top').outerHeight() + 47);
        } else {
          _this.currentBuilder.css('height', 0);
        }
        _this.updateAvailableComponents();
        _this.elementsSortable();
        $(window).trigger('ehfb.grid.ready');
        _this.builderGridContentFlag = false;
      }, _this.updateGridDelay);
    },
      ...
      sections.forEach(function (section) {
        if (typeof wp.customize.section(section) !== 'undefined') {
          wp.customize.section(section).expanded.bind(function (is_active) {
            ...
            self.currentBuilderType = self.currentBuilder.hasClass('emoza-ehfb-header') ? 'header' : 'footer';
            if (is_active) {
              $('body').addClass('ehfb-active');
              self.currentBuilder.addClass('show');
              self.scrollToRespectiveBuilderArea();
            } else {
              $('body').removeClass('ehfb-active');
            ...
            setTimeout(function () {
              ...
              if (section === 'emoza_section_hb_wrapper' || section === 'emoza_section_fb_wrapper') {
                // $( '.emoza-ehfb-' + self.currentBuilderType ).find( '.emoza-ehfb-above-row .emoza-ehfb-area' ).trigger( 'click' );
    ...
    getCurrentBuilderByComponent: function getCurrentBuilderByComponent(component) {
      if (component.indexOf('_hb_') !== -1 || component.indexOf('_header_') !== -1 || component.indexOf('header_image') !== -1) {
        return $('.emoza-ehfb-header');
      } else if (component.indexOf('_fb_') !== -1 || component.indexOf('_footer_') !== -1) {
        return $('.emoza-ehfb-footer');
      }
      return false;
    },
    showHideBuilderTop: function showHideBuilderTop() {
      var self = this;
      $('.emoza-ehfb-bottom-display').on('click', function (e) {
        e.preventDefault();
        $('body').toggleClass('ehfb-active-bottom');
        $(this).toggleClass('show');
        $('.emoza-ehfb-top').toggleClass('show');
        $('.emoza-ehfb').toggleClass('show-bottom');
        self.builderGridContent();
      });
    },
              ...
              option.bind(function (to) {
                ...
                for (var _i7 = 0, _rows3 = rows; _i7 < _rows3.length; _i7++) {
                  ...
                  if (optionID.indexOf(rowOptionID) !== -1) {
                    rowSelector = 'emoza-ehfb-' + row + '-row';
              ...
              wp.customize.section(sectionID).expanded.bind(function (is_active) {
                if (is_active) {
                  ...
                  var _loop6 = function _loop6() {
                    ...
                    setTimeout(function () {
                      var rowSelector = 'emoza-ehfb-' + row + '-row',
    ...
    addBuilderRowColumnsClass: function addBuilderRowColumnsClass(device, rowSelector, to) {
      ...
      for (var i = 1; i <= 6; i++) {
        $('.emoza-ehfb-' + _this.currentBuilderType + ' .emoza-ehfb-' + device + ' .emoza-ehfb-row.' + rowSelector).removeClass('emoza-ehfb-row-' + i + '-columns');
      }

      // Add new columns class.
      $('.emoza-ehfb-' + _this.currentBuilderType + ' .emoza-ehfb-' + device + ' .emoza-ehfb-row.' + rowSelector).addClass('emoza-ehfb-row-' + to + '-columns');
    },
    updateColumnsLayoutOption: function updateColumnsLayoutOption(device, val) {
      ...
      if ($('label[for="' + selector + '"]').parent().hasClass('ehfb-option-updated')) {
        return false;
      }
      ...
      // Add class as a flag.
      $('label[for="' + selector + '"]').parent().addClass('ehfb-option-updated');
    },
          ...
          var _loop7 = function _loop7() {
            ...
            wp.customize(optionID + deviceSelector, function (option) {
              option.bind(function (to) {
                ...
                var $builderRow = $('.emoza-ehfb-' + _this.currentBuilderType + ' .emoza-ehfb-' + device + ' .emoza-ehfb-row.emoza-ehfb-' + current_row + '-row');
                $builderRow.removeClass('emoza-ehfb-row-columns-layout-equal');
                $builderRow.removeClass('emoza-ehfb-row-columns-layout-bigleft');
                $builderRow.removeClass('emoza-ehfb-row-columns-layout-bigright');
                if (to.indexOf('equal') !== -1) {
                  $builderRow.addClass('emoza-ehfb-row-columns-layout-equal');
                }
                if (to.indexOf('bigleft') !== -1) {
                  $builderRow.addClass('emoza-ehfb-row-columns-layout-bigleft');
                }
                if (to.indexOf('bigright') !== -1) {
                  $builderRow.addClass('emoza-ehfb-row-columns-layout-bigright');
                }
    ...
    updateAvailableColumnsArea: function updateAvailableColumnsArea(device, colsNumber) {
      var _this = this,
        ...
        avCompsItems = rowSection.find('.ehfb-available-columns.ehfb-available-columns-' + device + ' .ehfb-available-columns-item');
