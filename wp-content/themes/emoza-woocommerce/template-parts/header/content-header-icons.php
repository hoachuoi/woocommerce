...
<?php if ( $show_account ) {
    echo '<a class="header-item wc-account-link" href="' . esc_url( get_permalink( get_option('woocommerce_myaccount_page_id') ) ) . '" title="' . esc_html__( 'Your account', 'emoza-woocommerce' ) . '"><i class="ws-svg-icon">' . emoza_get_header_icon( 'account' ) . '</i></a>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
} ?>	
...
<?php if( $wishlist_enable && $show_wishlist && $enable_header_wishlist_icon ) : 
    ...
    <a class="header-item header-wishlist-icon" href="<?php echo esc_url( emoza_get_permalink( get_option( 'emoza_wishlist_page_id' ) ) ); ?>" title="<?php echo esc_attr__( 'Your wishlist', 'emoza-woocommerce' ); ?>">
